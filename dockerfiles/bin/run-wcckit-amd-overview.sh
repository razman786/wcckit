...
usage() {
    cat <<'EOF'
Run the WCCKIT Pipeline Overview collector with AMD μProf hardware counters.
...
Usage:
  run-wcckit-amd-overview.sh --pid PID [options]
  run-wcckit-amd-overview.sh --match PATTERN [options]

Common examples:
  run-wcckit-amd-overview.sh --match DDFacet --pipeline DDFacet --language python \
    --influx-url http://127.0.0.1:18086

  run-wcckit-amd-overview.sh --pid 12345 --pipeline MyPipeline --language python \
    --influx-url http://127.0.0.1:18086
...
[[ -x "${OVERVIEW}" ]] || { printf '[wcckit-amd-uprof] error: missing overview wrapper: %s\n' "${OVERVIEW}" >&2; exit 1; }

exec "${OVERVIEW}" --hardware-counters amd-uprof "$@"
