...
usage() {
    cat <<'EOF'
Collect first-round WCCKIT pipeline telemetry with BCC/eBPF, Intel PCM, and
...
warn() { printf '[wcckit-pipeline] warning: %s\n' "$*" >&2; }
...
json_manifest() {
    env RUN_ID="${RUN_ID}" PID="${PID}" DURATION="${DURATION}" PIPELINE="${PIPELINE}" LANGUAGE="${LANGUAGE}" \
        ...
        INFLUX_BUCKET="${INFLUX_BUCKET}" INFLUX_TOKEN="${INFLUX_TOKEN}" PCM="${PCM}" BPF_IO="${BPF_IO}" APP_STAT="${APP_STAT}" \
        APP_CALLS="${APP_CALLS}" APP_FLOW_SUMMARY="${APP_FLOW_SUMMARY}" APP_FLOW_RAW="${APP_FLOW_RAW}" \
        FLAMEGRAPH="${FLAMEGRAPH}" python3 - <<'PYMANIFEST' > "${RUN_DIR}/manifest.json"
...
manifest = {
    ...
    "collectors": {
        "pcm": os.environ["PCM"] == "1",
        "bpf_io": os.environ["BPF_IO"] == "1",
        "app_stat": os.environ["APP_STAT"] == "1",
        "app_calls": os.environ["APP_CALLS"] == "1",
        "app_flow_summary": os.environ["APP_FLOW_SUMMARY"] == "1",
        "app_flow_raw": os.environ["APP_FLOW_RAW"] == "1",
        "flamegraph": os.environ["FLAMEGRAPH"] == "1",
    },
    "versions": {"pcm": cmd(["pcm", "--version"]), "python": cmd(["python3", "--version"]), "perf": cmd(["perf", "--version"])},
}
...
append_collector_status() {
    local status_file name status ok ts
    for status_file in "${LOGS_DIR}"/*.status; do
        ...
        [[ -n "${status}" ]] || status=255
        ok=false
        if [[ "${status}" -eq 0 ]]; then ok=true; else warn "collector ${name} exited with status ${status}"; fi
        ts="$(date +%s%N)"
...
require_cmd timeout
require_cmd curl
[[ "${PCM}" -eq 0 ]] || require_cmd pcm-sensor-server
[[ "${BPF_IO}" -eq 0 ]] || require_cmd biosnoop-bpfcc
...
log "pipeline=${PIPELINE} pid=${PID} language=${LANGUAGE} duration=${DURATION}s"
...
if [[ "${BPF_IO}" -eq 1 ]]; then start_bg bpf-io timeout --foreground "${DURATION}" biosnoop-bpfcc; fi
